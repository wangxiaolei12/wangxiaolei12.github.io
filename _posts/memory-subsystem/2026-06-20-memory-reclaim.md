...
INACTIVE = 死刑候选区，从尾部开始执行；ACTIVE = 保护区，只有"冷了"才会被踢出去。
...
```
  ...
  └─────────────────────────────────────┘
                                    ...
                                    ▼
                               检查 Accessed bit：
                                 ├─ Accessed=1 → 留在ACTIVE（还热）
                                 └─ Accessed=0 → 踢回 INACTIVE 头部
                                                      │
                                                      ▼
                                                  回到 INACTIVE
                                                  重新排队等待回收...
...
页面的完整流动方向：`新生 → INACTIVE → (被访问) → ACTIVE → (没人用了) → INACTIVE → 被回收释放`

**关键点**：Accessed bit 是 CPU 硬件帮你设的。每次 CPU 通过页表访问一个页面，MMU 自动把该 PTE 的 Accessed 位置 1。内核定期来"检查并清零"，下次再来如果还是 0，说明在这段时间内确实没人访问——这就是时钟算法（Clock）的变体。

### 4.2.1 为什么分成 4 条链表：文件页 vs 匿名页

流动方式一样（都是 inactive↔active），但**回收代价完全不同**：
...
│  来源：读文件、mmap文件、代码段                               │
│  回收方式：                                                  │
│    clean → 直接丢掉（磁盘上有原始副本，随时能再读回来）        │
│    dirty → writeback写回磁盘 → 丢掉                          │
│                                                             │
│  代价：低（大部分是clean的，零代价释放）                       │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│  匿名页（anon）                                              │
│                                                             │
│  来源：malloc、栈、堆、私有mmap写入后的COW页                   │
│  回收方式：                                                  │
│    必须先写到 swap 分区/文件 → 才能丢掉                       │
│    （因为磁盘上没有原始副本，丢了就没了）                      │
│                                                             │
│  代价：高（必须产生磁盘IO写swap）                             │
│  没有swap？→ 根本无法回收，只能OOM                            │
...
**所以内核默认策略：优先回收文件页，尽量少动匿名页。**

分成 4 条链表的好处：可以独立控制"回收多少文件页 vs 多少匿名页"，通过 `swappiness` 参数调节（默认 60，越大越愿意 swap 匿名页）。

```
INACTIVE_FILE ←→ ACTIVE_FILE     ← 文件页自己的老化/回收循环
INACTIVE_ANON ←→ ACTIVE_ANON     ← 匿名页自己的老化/回收循环
```
