...
### 10.1 FS 和 FE_ACK 的含义

```
Sensor 发送：  [FS短包] → [行数据×N] → [FE短包]
                 │                        │
Host 硬件：     FS 中断                  FE_ACK 中断
                 │                        │
含义：          "帧开始了"              "帧结束，DMA写完了"
```

- **FS（Frame Start）**：CSI2 Host 收到帧开始短包，sensor 开始发新一帧
- **FE_ACK（Frame End Acknowledge）**：收到帧结束短包，且 DMA 已把整帧数据搬完

**有 FS 没 FE_ACK = 帧传输中途断了**，可能原因：

| 原因 | 说明 |
|------|------|
| Lane 配置错误 | sensor 用 4 lane 发，驱动只配了 2 lane |
| D-PHY 速率不对 | 采样错误导致后续数据全是乱码 |
| sensor 异常 | I2C 掉了、供电问题 |
| 信号完整性 | 排线太长/干扰，CRC 出错 |

### 10.2 开启驱动日志

```bash
# 开启 verbose 日志和错误追踪
echo 1 > /sys/module/rp1_cfe/parameters/verbose_debug
echo 1 > /sys/module/rp1_cfe/parameters/track_csi2_errors
dmesg -w | grep csi2
```

正常输出：
```
ISR: [0], FS FE_ACK frame: 42 line: 0    ← 有开始有结束，正常
```

异常输出：
```
ISR: [0], FS frame: 43 line: 0            ← 只有开始没结束，丢帧
```

### 10.3 debugfs 错误统计

```bash
cat /sys/kernel/debug/rp1-cfe*/csi2_errors
```

```
Overflows 5
Discards:
VC            OVLF        LEN  UNMATCHED   INACTIVE
0              0          0       12          0
```

| 错误 | 含义 | 排查方向 |
|------|------|---------|
| Overflow | FIFO 满 | DDR带宽不足、帧率太高 |
| Discard LEN_LIMIT | 包长超限 | sensor 输出格式与驱动配置不匹配 |
| Discard UNMATCHED | 无通道匹配 VC+DT | sensor 发的 VC/DT 和驱动配置对不上 |
| Discard INACTIVE | 通道未激活 | 启动时序问题，sensor 先于 host 发数据 |

### 10.4 debugfs 寄存器状态

```bash
cat /sys/kernel/debug/rp1-cfe*/csi2_regs
```

```
CSI2_CH_CTRL(0) 0x00006b01    ← Channel 0: DMA使能, DT=0x2B(RAW10), VC=0
CSI2_CH_DEBUG(0) 0x002a0000   ← frame=42, line=0
```
...
没图像？
  ...
  │
  ├─ 有 UNMATCHED discard？
  │     → VC/DT 配置不一致，对比 sensor datasheet
  │
  ├─ 有 Overflow？
  │     → DMA 搬不过来，降分辨率/帧率
  │
  └─ 图像花屏/绿屏？
        → 格式不匹配（如 sensor 发 RAW10 驱动按 RAW8 收）
```
...
| 节点 | 方向 | 作用 |
|------|------|------|
| `/dev/video4` (fe_image0) | FE → 用户 | **主输出图像**：经 ISP 处理后的图（去噪、白平衡等），全分辨率 |
| `/dev/video5` (fe_image1) | FE → 用户 | **第二路输出图像**：缩小版/不同格式（如预览用低分辨率） |
| `/dev/video6` (fe_stats) | FE → 用户 | **统计信息**：AE/AWB/AF 统计数据（亮度直方图、色温等），给 3A 算法用 |
| `/dev/video7` (fe_config) | 用户 → FE | **配置输入**：用户空间写入 ISP 参数（增益、色彩矩阵、去噪强度等） |
...
### 11.3 典型使用流程（libcamera 3A 循环）

1. 每帧开始前：用户通过 `video7` 写入本帧的 ISP 配置（根据上一帧统计调整）
2. FE 收到 RAW 图像后按配置处理
3. 从 `video4` 拿处理后的图用于拍照/录像
4. 从 `video5` 拿低分辨率图用于取景器预览
5. 从 `video6` 拿统计信息，3A 算法据此计算下一帧的曝光/白平衡/对焦参数
6. 把新参数写入 `video7`，循环往复
