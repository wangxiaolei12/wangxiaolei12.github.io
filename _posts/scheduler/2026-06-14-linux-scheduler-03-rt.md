...
FIFO 和 RR 的区分 **不是通过不同的队列**，而是存在进程自身的 `task_struct` 中：

```c
struct task_struct {
    unsigned int policy;  // 调度策略
    // SCHED_NORMAL = 0  (CFS)
    // SCHED_FIFO   = 1  (RT 先进先出)
    // SCHED_RR     = 2  (RT 时间片轮转)
    // ...
};
```
...
sched_setscheduler(pid, SCHED_FIFO, &param);  // → p->policy = SCHED_FIFO
sched_setscheduler(pid, SCHED_RR, &param);    // → p->policy = SCHED_RR
```

### FIFO 和 RR 在同一条链表里

两种策略的进程 **共享同一个优先级链表**，入队时都插到同一个 `queue[prio]` 链表：

```
prio=50 的链表: [FIFO_A] → [RR_B] → [RR_C] → [FIFO_D]
                   ↑
              pick 时取这个（链表头）
```

**区分行为的唯一地方是 tick 处理**——FIFO 不减时间片不轮转，RR 会。详见下面第五节的 `task_tick_rt()` 源码分析。

### 举例：混合场景

3 个 RT 进程，优先级都是 50，A 是 FIFO，B 和 C 是 RR：

```
初始链表: [FIFO_A] → [RR_B] → [RR_C]

1. 选中 A（链表头）
2. A 是 FIFO → tick 时 task_tick_rt() 检查 policy != SCHED_RR → 直接 return
   → A 永不被轮转，一直运行
3. A 主动 sleep → 链表变为 [RR_B] → [RR_C]
4. 选中 B（链表头）
5. B 是 RR → 每 tick 减 time_slice
6. B 时间片到期(100ms) → requeue 到末尾 → [RR_C] → [RR_B]
7. 选中 C，运行 100ms → requeue → [RR_B] → [RR_C]
8. B 和 C 轮流跑...
9. A 被唤醒 → 入队到链表尾 → [RR_B] → [RR_C] → [FIFO_A]
   （注意：唤醒默认插链表尾，但 A 优先级相同不抢占当前进程）
10. 等 B 时间片到期轮转，最终 A 到链表头后霸占 CPU
```

### 纯 SCHED_FIFO 举例

3 个 FIFO 进程，优先级都是 50：
```
...
## 五、task_tick_rt——时钟 tick 处理（FIFO 和 RR 的唯一区别点）

每次 tick 中断时，调度器调用 `task_tick_rt()`。这是 **FIFO 和 RR 行为差异的唯一代码路径**：

```c
// kernel/sched/rt.c 第 2534 行
static void task_tick_rt(struct rq *rq, struct task_struct *p, int queued)
{
     ...
     */
    if (p->policy != SCHED_RR)   // ★ FIFO 在这里直接 return！
        return;                   //   不减时间片，不轮转，什么都不做

    // ═══ 以下代码只有 SCHED_RR 才会执行 ═══

    if (--p->rt.time_slice)      // 时间片减 1，没到 0 就继续运行
        return;
...
| 行 | 代码 | 说明 |
|---|------|------|
| 2547 | `if (p->policy != SCHED_RR) return;` | **关键！** 检查 `p->policy`，FIFO 直接返回 |
| 2549 | `if (--p->rt.time_slice) return;` | RR 专属：每 tick 减 1，非零继续 |
| 2552 | `p->rt.time_slice = sched_rr_timeslice;` | 时间片归零后重填（100ms） |
| 2560 | `run_list.prev != run_list.next` | 链表不止一个元素？ |
| 2561 | `requeue_task_rt(rq, p, 0)` | 是 → 当前进程移到链表末尾 |
| 2562 | `resched_curr(rq)` | 标记需要重调度，中断返回时切换 |

### requeue_task_rt 做了什么

```c
// kernel/sched/rt.c 第 1472 行
requeue_rt_entity(struct rt_rq *rt_rq, struct sched_rt_entity *rt_se, int head)
{
    ...
    if (head)
        list_move(&rt_se->run_list, queue);       // 移到链表头
    else
        list_move_tail(&rt_se->run_list, queue);  // 移到链表尾（RR 轮转用这个）
}
```

`list_move_tail` 就是把当前进程从链表当前位置摘下来，挂到尾巴上。下次 `pick_next_task_rt` 取链表头时，就取到了下一个进程。
